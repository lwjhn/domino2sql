...
public class DSProcessDispatch implements DSProcess {
    ...
    public void process(Vector dsItem, Document doc, Session session) throws Exception {
        ...
        if (doc != null && session != null) {
            ...
            if(mssServer==null || "".equals(mssServer))
                mssServer = doc.getParentDatabase().getServer();
            ...
            if (StringUtils.isNoneBlank(mssOpinion)) {
    ...
    private DSItem initValue(DSItem item) {
        try {
            ...
            Vector v = session.evaluate(rawVal == null || rawVal.length() < 1 ? name : item.getValue(), document); // 如果域值有计算公式，那么处理计算公式并返回结果
...
//            }
    ...
    private String getOpinion(String itemName, String format) throws Exception {
        ...
        while (entry != null) {
            ...
            if (itemName!=null && itemName.equals(opinionDoc.getItemValueString("OpinionField")) || itemName.equals(opinionDoc.getItemValueString("OpinionType"))) {
